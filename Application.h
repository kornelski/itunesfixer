...
@class ApplicationPrintSettings, ApplicationApplication, ApplicationItem, ApplicationArtwork, ApplicationEncoder, ApplicationEQPreset, ApplicationPlaylist, ApplicationAudioCDPlaylist, ApplicationDevicePlaylist, ApplicationLibraryPlaylist, ApplicationRadioTunerPlaylist, ApplicationSource, ApplicationTrack, ApplicationAudioCDTrack, ApplicationDeviceTrack, ApplicationFileTrack, ApplicationSharedTrack, ApplicationURLTrack, ApplicationUserPlaylist, ApplicationFolderPlaylist, ApplicationVisual, ApplicationWindow, ApplicationBrowserWindow, ApplicationEQWindow, ApplicationPlaylistWindow;

enum ApplicationEKnd {
	ApplicationEKndTrackListing = 'kTrk' /* a basic listing of tracks within a playlist */,
	ApplicationEKndAlbumListing = 'kAlb' /* a listing of a playlist grouped by album */,
	ApplicationEKndCdInsert = 'kCDi' /* a printout of the playlist for jewel case inserts */
};
typedef enum ApplicationEKnd ApplicationEKnd;
...
enum ApplicationEPlS {
	ApplicationEPlSStopped = 'kPSS',
	ApplicationEPlSPlaying = 'kPSP',
	ApplicationEPlSPaused = 'kPSp',
	ApplicationEPlSFastForwarding = 'kPSF',
	ApplicationEPlSRewinding = 'kPSR'
};
typedef enum ApplicationEPlS ApplicationEPlS;

enum ApplicationERpt {
	ApplicationERptOff = 'kRpO',
	ApplicationERptOne = 'kRp1',
	ApplicationERptAll = 'kAll'
};
typedef enum ApplicationERpt ApplicationERpt;

enum ApplicationEVSz {
	ApplicationEVSzSmall = 'kVSS',
	ApplicationEVSzMedium = 'kVSM',
	ApplicationEVSzLarge = 'kVSL'
};
typedef enum ApplicationEVSz ApplicationEVSz;

enum ApplicationESrc {
	ApplicationESrcLibrary = 'kLib',
	ApplicationESrcIPod = 'kPod',
	ApplicationESrcAudioCD = 'kACD',
	ApplicationESrcMP3CD = 'kMCD',
	ApplicationESrcDevice = 'kDev',
	ApplicationESrcRadioTuner = 'kTun',
	ApplicationESrcSharedLibrary = 'kShd',
	ApplicationESrcUnknown = 'kUnk'
};
typedef enum ApplicationESrc ApplicationESrc;

enum ApplicationESrA {
	ApplicationESrAAlbums = 'kSrL' /* albums only */,
	ApplicationESrAAll = 'kAll' /* all text fields */,
	ApplicationESrAArtists = 'kSrR' /* artists only */,
	ApplicationESrAComposers = 'kSrC' /* composers only */,
	ApplicationESrADisplayed = 'kSrV' /* visible text fields */,
	ApplicationESrASongs = 'kSrS' /* song names only */
};
typedef enum ApplicationESrA ApplicationESrA;

enum ApplicationESpK {
	ApplicationESpKNone = 'kNon',
	ApplicationESpKAudiobooks = 'kSpA',
	ApplicationESpKFolder = 'kSpF',
	ApplicationESpKMovies = 'kSpI',
	ApplicationESpKMusic = 'kSpZ',
	ApplicationESpKPartyShuffle = 'kSpS',
	ApplicationESpKPodcasts = 'kSpP',
	ApplicationESpKPurchasedMusic = 'kSpM',
	ApplicationESpKTVShows = 'kSpT',
	ApplicationESpKVideos = 'kSpV'
};
typedef enum ApplicationESpK ApplicationESpK;

enum ApplicationEVdK {
	ApplicationEVdKNone = 'kNon' /* not a video or unknown video kind */,
	ApplicationEVdKMovie = 'kVdM' /* movie track */,
	ApplicationEVdKMusicVideo = 'kVdV' /* music video track */,
	ApplicationEVdKTVShow = 'kVdT' /* TV show track */
};
typedef enum ApplicationEVdK ApplicationEVdK;

enum ApplicationERtK {
	ApplicationERtKUser = 'kRtU' /* user-specified rating */,
	ApplicationERtKComputed = 'kRtC' /* iTunes-computed rating */
};
typedef enum ApplicationERtK ApplicationERtK;
...
@property (readonly) ApplicationEnum errorHandling;  // how errors are handled
...
- (void) printPrintDialog:(BOOL)printDialog withProperties:(ApplicationPrintSettings *)withProperties kind:(ApplicationEKnd)kind theme:(NSString *)theme;  // Print the specified object(s)
...
@property (copy) ApplicationEncoder *currentEncoder;  // the currently selected encoder (MP3, AIFF, WAV, etc.)
@property (copy) ApplicationEQPreset *currentEQPreset;  // the currently selected equalizer preset
@property (copy, readonly) ApplicationPlaylist *currentPlaylist;  // the playlist containing the currently targeted track
...
@property (copy, readonly) ApplicationTrack *currentTrack;  // the current targeted track
@property (copy) ApplicationVisual *currentVisual;  //  the currently selected visual plug-in
...
@property (readonly) ApplicationEPlS playerState;  // is iTunes stopped, paused, or playing?
...
@property ApplicationEVSz visualSize;  // the size of the displayed visual

- (void) printPrintDialog:(BOOL)printDialog withProperties:(ApplicationPrintSettings *)withProperties kind:(ApplicationEKnd)kind theme:(NSString *)theme;  // Print the specified object(s)
- (void) run;  // run iTunes
- (void) quit;  // quit iTunes
- (ApplicationTrack *) add:(NSArray *)x to:(SBObject *)to;  // add one or more files to a playlist
- (void) backTrack;  // reposition to beginning of current track or go to previous track if already at start of current track
- (ApplicationTrack *) convert:(NSArray *)x;  // convert one or more files or tracks
...
@interface ApplicationArtwork : ApplicationItem
...
@interface ApplicationEncoder : ApplicationItem
...
@interface ApplicationEQPreset : ApplicationItem
...
@interface ApplicationPlaylist : ApplicationItem
...
@property (copy, readonly) ApplicationPlaylist *parent;  // folder which contains this playlist (if any)
...
@property ApplicationERpt songRepeat;  // playback repeat mode
@property (readonly) ApplicationESpK specialKind;  // special playlist kind
...
- (ApplicationTrack *) searchFor:(NSString *)for_ only:(ApplicationESrA)only;  // search a playlist for tracks matching the search string. Identical to entering search text in the Search field in iTunes.

@end

// a playlist representing an audio CD
@interface ApplicationAudioCDPlaylist : ApplicationPlaylist
...
@interface ApplicationDevicePlaylist : ApplicationPlaylist
...
@interface ApplicationLibraryPlaylist : ApplicationPlaylist
...
@interface ApplicationRadioTunerPlaylist : ApplicationPlaylist
...
@interface ApplicationSource : ApplicationItem
...
@property (readonly) ApplicationESrc kind;
...
@property (readonly) ApplicationERtK albumRatingKind;  // the rating kind of the album rating for this track
...
@property (readonly) ApplicationERtK ratingKind;  // the rating kind of this track
...
@property ApplicationEVdK videoKind;  // kind of video track
...
@interface ApplicationAudioCDTrack : ApplicationTrack
...
@interface ApplicationDeviceTrack : ApplicationTrack


@end

// a track representing an audio file (MP3, AIFF, etc.)
@interface ApplicationFileTrack : ApplicationTrack
...
@interface ApplicationSharedTrack : ApplicationTrack


@end

// a track representing a network stream
@interface ApplicationURLTrack : ApplicationTrack
...
@interface ApplicationUserPlaylist : ApplicationPlaylist
...
@interface ApplicationFolderPlaylist : ApplicationUserPlaylist


@end

// a visual plug-in
@interface ApplicationVisual : ApplicationItem
...
@interface ApplicationBrowserWindow : ApplicationWindow

@property BOOL minimized;  // is the small player visible?
@property (copy, readonly) SBObject *selection;  // the selected songs
@property (copy) ApplicationPlaylist *view;  // the playlist currently displayed in the window


@end

// the iTunes equalizer window
@interface ApplicationEQWindow : ApplicationWindow
...
@interface ApplicationPlaylistWindow : ApplicationWindow

@property (copy, readonly) SBObject *selection;  // the selected songs
@property (copy, readonly) ApplicationPlaylist *view;  // the playlist displayed in the window
